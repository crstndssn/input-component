(function() {
    ...
    template.innerHTML = `
        ...
        <ul></ul>
        <input type="text" placeholder="Type or paste email addresses and press 'Enter'...">
        <p hidden></p>
    `;
    ...
    class BadgeInput extends HTMLElement{
        ...
        handlePaste = evt => {
            ...
            var paste = evt.clipoboardData.getData("text")
            var emails = paste.match(/[\w\d\.-]+@[\w\d\.-]+\.[\w\d\.-]+/g);

            if (emails) {
                var toBeAdded = email.filter(email => !this.isInList(email));
    ...
    window.customElements.define("badge-input", BadgeInput);
